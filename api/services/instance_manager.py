...
class InstanceManager:
    ...
    async def delete_instance(self, instance_id: str) -> bool:
        ...
        if self._docker:
            try:
                ...
                logger.info("Removed container assistantx-%s", instance_id)
            except docker.errors.NotFound:
                pass
            except Exception as exc:
                logger.warning("Remove container assistantx-%s: %s", instance_id, exc)
    ...
    async def _provision_docker(self, instance: Instance, config: InstanceConfig):
        ...
        try:
            ...
            logger.info("Starting OpenClaw gateway in container assistantx-%s on host port %d ...", instance.id, host_port)
            ...
            for attempt in range(15):
                ...
                try:
                    ...
                    async with websockets.connect(
                        ...
                        gateway_ready = True
                        logger.info("Gateway ready for assistantx-%s after %ds", instance.id, (attempt+1)*2)
                        break
                except Exception:
                    pass

            if not gateway_ready:
                logger.warning("Gateway not ready for assistantx-%s after 30s — falling back to docker exec", instance.id)
    ...
    async def _provision_profile(self, instance: Instance, config: InstanceConfig):
        ...
        try:
            ...
            instance.container_id = f"profile:assistantx-{instance.id}"
            instance.status = InstanceStatus.RUNNING
            self._save_state()
